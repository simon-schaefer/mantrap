...
from mantrap.solver import IGradSolver, SGradSolver, MonteCarloTreeSearch, ORCASolver
...
def solver_igrad_t2() -> Tuple[Solver.__class__, Dict[str, Any]]:
    return IGradSolver, {"t_planning": 2}
...
if __name__ == '__main__':
    ...
    for (scenario_name, scenario_func), (solver_name, solver_func) in product(scenarios.items(), solvers.items()):
        ...
        x_opt, ado_states, x_opt_planned = solver.solve(time_steps=20, max_cpu_time=0.5, multiprocessing=True)

        exp_tag = f"{solver_name}:{scenario_name}"
        x_ados = ado_states[:, :, :, 0, :].permute(1, 2, 0, 3)  # just actual ado trajectory (not predicted ones)
        results[exp_tag] = evaluate_metrics(ego_trajectory=x_opt, ado_trajectories=x_ados)
        visualize_scenes(x_opt_planned, ado_states, env=sim, file_path=build_os_path(f"test/graphs/{exp_tag}"))
