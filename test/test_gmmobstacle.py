...
import murseco.obstacle.gmmobstacle
...
def test_gmmobstacle_tmax():
    tmus = np.ones((1, 2, 2))
    tsigmas = np.ones((1, 2, 2, 2))
    tweights = np.ones((1, 2))
    obstacle = murseco.obstacle.gmmobstacle.GMMDiscreteTimeObstacle(tmus, tsigmas, tweights)
    obstacle.forward()
...
@pytest.mark.parametrize(
    "tmus, tsigmas, tweights",
    [
        (np.random.rand(2, 2, 2), rand_invsymmpos(2, 2, 2, 2), np.random.rand(2, 2)),
    ],
)
def test_stats_gmm_obstacle_json(tmus: np.ndarray, tsigmas: np.ndarray, tweights: np.ndarray):
    gmmobstacle_1 = murseco.obstacle.gmmobstacle.GMMDiscreteTimeObstacle(tmus, tsigmas, tweights)
    cache_path = murseco.utility.io.path_from_home_directory("test/cache/gmmobstacle_test.json")
    gmmobstacle_1.to_json(cache_path)
    gmmobstacle_2 = murseco.obstacle.gmmobstacle.GMMDiscreteTimeObstacle.from_json(cache_path)
