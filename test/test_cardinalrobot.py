...
def test_cardinalrobot_trajectory():
    ...
    trajectory = CardinalDiscreteTimeRobot(position, thorizon, pstep, policy).trajectory
...
def test_cardinalrobot_json():
    ...
    robot_1 = CardinalDiscreteTimeRobot(position, thorizon, pstep, policy)
    cache_path = murseco.utility.io.path_from_home_directory("test/cache/cardinalrobot_test.json")
    robot_1.to_json(cache_path)
    robot_2 = CardinalDiscreteTimeRobot.from_json(cache_path)
