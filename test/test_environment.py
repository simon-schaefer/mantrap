...
def test_environment_identifier():
    ...
    identifiers = []
    for i in range(10):
        identifier = env.add_obstacle(CardinalDiscreteTimeObstacle(sigmas=rand_invsymmpos(4, 2, 2)))
...
def test_environment_plot():
    ...
    env.add_obstacle(CardinalDiscreteTimeObstacle(pinit=np.ones(2) * 3))
    env.add_obstacle(CardinalDiscreteTimeObstacle(pinit=np.ones(2) * (-4)))
...
def test_environment_json():
    ...
    env_1.add_obstacle(CardinalDiscreteTimeObstacle(pinit=np.array([1.4, 4.2])))
    env_1.add_obstacle(CardinalDiscreteTimeObstacle(pinit=np.array([5.4, -2.94])))
...
def test_environment_same_tmax():
    env = Environment((-10, 10), (-10, 10))
    env.add_obstacle(CardinalDiscreteTimeObstacle(tmax=5))
    env.add_obstacle(CardinalDiscreteTimeObstacle(tmax=3))
...
def test_environment_visualization_all_times():
    ...
    env.add_obstacle(CardinalDiscreteTimeObstacle(np.zeros(2), 1.0, thorizon, sigmas, weights))
