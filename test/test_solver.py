...
def visualize_rrt_single_static():
    sim = scenarios.sf_ego_static_single_ado(ego_type=DoubleIntegratorDTAgent)
...
def visualize_rrt_single_moving():
    sim = scenarios.sf_ego_moving_single_ado(ego_type=DoubleIntegratorDTAgent)
    solver = TrajOptSolver(sim, goal=np.array([5, 0.1]))
    ego_traj, ados_traj = solver.solve()
...
def visualize_rrt_some():
    sim = scenarios.sf_ego_moving_two_ados(ego_type=DoubleIntegratorDTAgent)
    solver = TrajOptSolver(sim, goal=np.array([5, 0.1]))
...
def visualize_rrt_many():
    sim = scenarios.sf_ego_moving_many_ados(ego_type=DoubleIntegratorDTAgent)
    solver = TrajOptSolver(sim, goal=np.array([5, 0.1]))
    ego_trajectory, ado_trajectories = solver.solve()
