    ...
    def add_ado(
        ...
        **ado_kwargs
        ...
        if type(v0s) != np.ndarray:
            v0s, weights = self.ado_mode_params(v0s, SOCIAL_FORCES_DEFAULTS[PK_V0], num_modes=num_modes)

        # Fill ghost argument list with mode parameters.
        args_list = [{PK_V0: v0s[i]} for i in range(num_modes)]
    ...
    def build_graph(self, ego_state: torch.Tensor = None, k: int = 0,  **graph_kwargs) -> Dict[str, torch.Tensor]:
        ...
        for ghost in self.ghosts:
            ...
            if ego_state is not None:
                ...
                delta = gpos - ego_pos
                force = ghost.params[PK_V0] * torch.sign(delta) * torch.exp(- torch.abs(delta)) * ghost.weight
