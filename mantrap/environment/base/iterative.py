...
class IterativeEnvironment(mantrap.environment.GraphBasedEnvironment, abc.ABC):
    ...
    ###########################################################################
    def _build_connected_graph(self, ego_trajectory: typing.Union[typing.List, torch.Tensor], **kwargs
                               ) -> typing.Dict[str, torch.Tensor]:
        ...
        for m_ghost in range(self.num_ghosts):
            ...
            action = graph[f"{ghost_id}_{t_horizon - 1}_{mantrap.constants.GK_CONTROL}"]
            self._ado_ghosts[m_ghost].agent.update(action, dt=self.dt)
            graph[f"{ghost_id}_{t_horizon}_{mantrap.constants.GK_POSITION}"] = self.ghosts[m_ghost].agent.position
            graph[f"{ghost_id}_{t_horizon}_{mantrap.constants.GK_VELOCITY}"] = self.ghosts[m_ghost].agent.velocity
